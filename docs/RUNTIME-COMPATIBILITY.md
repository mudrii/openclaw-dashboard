...
| Provider plans and balances | Gateway `usage.status` provides reported quota windows, reset times, plan names, and billing balances. The `providerUsage` collection preserves source timestamps and stale state. These values are distinct from 30-day recorded session spend; missing prices are not zero-cost usage. |
| Container gateway health | Gateway `health` provides a boolean verdict through the selected CLI/container target. The `gatewayHealth` collection drives Online/Unhealthy only while ready and complete; stale/unavailable results cannot leave an old Online status visible. Native host probes remain separate. |
...
The session summary separates active loaded sessions from the reported stored total. Token-table totals sum raw per-model components (cache column is cache-read; total includes the upstream total-token definition); usage bars represent token volume, not price. Complete pricing populates cost breakdowns. Incomplete pricing keeps total cost unknown, with the available dollar amount displayed under a known-subtotal label and unpriced-model counts explained explicitly. When today’s token collection is complete, the monthly card can show a partial estimate of today’s known cost multiplied by 30, explicitly excluding unpriced usage. The overview donut shows positive known costs by model as a partial breakdown; it does not assign prices to unpriced records. Daily token volume remains chartable when prices are incomplete; cost/model-cost charts do not invent prices. Runtime skill inventory takes precedence over explicit legacy overrides, and channel configuration cards preserve separate runtime accounts. Absent cron duration remains null rather than zero. See the [field-fix validation record](plans/2026-09-05-field-fixes.md) for implementation versus live-verification status.
