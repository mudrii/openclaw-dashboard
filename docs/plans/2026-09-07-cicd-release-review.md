...
4. **Installed Homebrew behavior was not tested.** Linux/macOS CI installs the generated formula from local archives using a unique keg-only identity, runs its formula test, and verifies runtime seeding, refresh, timezone and private snapshots. It retains the generated install logic and preserves any existing dashboard installation. Auto-update, cleanup and autoremove are disabled.
...
7. **SBOM tooling floated independently of its action pin.** Syft is pinned to v1.51.1 in both rehearsal and publication; Cosign is explicitly pinned to the installer default v3.0.6. Rehearsals generate all four SPDX SBOMs, verify their checksums and require dashboard/Go-runtime entries. Tool-version parity tests prevent CI/release drift.
8. **Release documentation and required-check guidance lagged behavior.** Updated contributor/install instructions, the PR template, changelog and infrastructure checklist. Removed publish/delete/retag rehearsal instructions and the undocumented signing-bypass recommendation. Publication keeps signing and SBOM generation enabled.
...
## Hosted checks and merge protection

All 11 Tests jobs passed on `7241b10` ([run 34050642162](https://github.com/mudrii/openclaw-dashboard/actions/runs/34050642162)), including Linux/macOS race tests, both Nix builds, container smoke and macOS Homebrew. The subsequent SBOM/Linux Homebrew additions require a fresh run on the final candidate. Main branch protection now retains its original required checks and additionally requires Staticcheck, Container smoke, both release rehearsals and both Nix checks; strict up-to-date checks remain enabled.

## Publication boundary

The candidate must pass the expanded hosted Tests workflow before it is considered ready to merge. The main branch must then pass that workflow for the merged commit before release tagging. Signing/OIDC and authenticated tap publication are exercised only by an authorized real release; the snapshot generates SBOMs and deliberately skips signing and never writes to the tap. Repository credential names were checked for presence, not private-key validity. Live OpenClaw inference and gateway mutations are outside these packaging fixtures.
