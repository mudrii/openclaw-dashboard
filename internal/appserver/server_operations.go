...
const (
	// minOperatorTokenLen is the shortest operator token that enables mutations.
	minOperatorTokenLen = 32
	// operationTimeout bounds the runtime reads and the mutation of one operation.
	operationTimeout = 35 * time.Second
	// maxOperationBodyBytes caps the JSON body of an operation request.
	maxOperationBodyBytes = 8192
)
...
func (s *Server) handleOperation(w http.ResponseWriter, r *http.Request) {
	...
	if !s.cfg.Operations.Enabled || len(s.operatorToken) < minOperatorTokenLen {
	...
	if !s.operationLimiter.allow("operator") {
		s.sendJSON(w, r, http.StatusTooManyRequests, map[string]string{"error": "rate_limited"})
		return
	}
	if strings.Split(r.Header.Get("Content-Type"), ";")[0] != "application/json" {
		s.sendJSON(w, r, http.StatusUnsupportedMediaType, map[string]string{"error": "json_required"})
		return
	}
	var request operationRequest
	d := json.NewDecoder(http.MaxBytesReader(w, r.Body, maxOperationBodyBytes))
	d.DisallowUnknownFields()
	err := d.Decode(&request)
	var extra any
	if err != nil || d.Decode(&extra) != io.EOF || !operationIDPattern.MatchString(request.OperationID) {
		s.sendJSON(w, r, http.StatusBadRequest, map[string]string{"error": "invalid_operation"})
		return
	}
	...
	case "automation.enable", "automation.disable", "automation.run":
		if !validRuntimeKey(request.ID) || strings.ContainsAny(request.ID, `/\\`) || !validRuntimeKey(request.ConfigRevision) {
			s.sendJSON(w, r, http.StatusBadRequest, map[string]string{"error": "exact_automation_revision_required"})
			return
		}
	case "session.abort":
		if !validRuntimeKey(request.SessionKey) || !validRuntimeKey(request.RunID) {
			s.sendJSON(w, r, http.StatusBadRequest, map[string]string{"error": "exact_session_run_required"})
			return
		}
	default:
		s.sendJSON(w, r, http.StatusBadRequest, map[string]string{"error": "unsupported_operation"})
		return
	}
	...
	if err := os.MkdirAll(auditDir, 0700); err != nil {
		s.sendJSON(w, r, http.StatusInternalServerError, map[string]string{"error": "audit_unavailable"})
		return
	}
	...
	ctx, cancel := context.WithTimeout(appopenclaw.WithTarget(r.Context(), s.cfg.Openclaw), operationTimeout)
	defer cancel()
	...
	if err := json.NewEncoder(audit).Encode(record); err != nil || audit.Sync() != nil {
		outcome, status, code = "unknown", http.StatusInternalServerError, "audit_unavailable"
	}
